...
```powershell
ssh-keygen -t ed25519 -f "$HOME\.ssh\id_ed25519_root" -C "root"
```
...
```powershell
Get-Content "$HOME\.ssh\id_ed25519_root.pub" | Set-Clipboard
```
...
```powershell
ssh -i "$HOME\.ssh\id_ed25519_root" root@192.168.1.99
```
...
```powershell
ssh-keygen -t ed25519 -f "$HOME\.ssh\id_ed25519_azzor" -C "azzor"
```
...
```powershell
Get-Content "$HOME\.ssh\id_ed25519_azzor.pub" | Set-Clipboard
```
...
```powershell
ssh -i "$HOME\.ssh\id_ed25519_azzor" azzor@192.168.1.99
```
...
```powershell
ssh -v -i "$HOME\.ssh\id_ed25519_root" root@192.168.1.99
```
...
```powershell
ssh -v -i "$HOME\.ssh\id_ed25519_azzor" azzor@192.168.1.99
```
...
> **Observação:** Dentro do arquivo `config`, o OpenSSH para Windows interpreta corretamente `~/.ssh`, portanto não é necessário utilizar `$HOME`.
